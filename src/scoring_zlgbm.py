...
def score_zlgbm_future_data(df: pl.DataFrame, config: Dict, campos_buenos: List[str]) -> pl.DataFrame:
    ...
    logger.info(f"Future data: {df_future.shape[0]:,} records from {future_months}")
    logger.info(f"Loading {ksemillerio} model(s)")
    ...
    for i, semilla in enumerate(semillas_to_use, 1):
        ...
        logger.info(f"Model {i}/{ksemillerio}: min={predictions.min():.6f}, max={predictions.max():.6f}")
    ...
    if ksemillerio > 1:
        ...
        logger.info(f"Ensemble averaged: std dev mean={std_predictions.mean():.6f}")
...
def generate_zlgbm_submission(df_pred: pl.DataFrame, config: Dict, n_envios: int = 11000) -> None:
    ...
    Args:
        ...
        config: Configuration dictionary
        n_envios: Number of top predictions to submit
    """
    ...
    logger.info(f"Submission: {n_predicted:,} positives, cutoff prob={prob_at_cutoff:.6f}")

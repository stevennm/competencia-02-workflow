...
print(f"Looking for anomalies in foto_mes 202006 (AFTER MICE imputation)...")
...
if anomalies:
    print(f"\n[WARNING] Found {len(anomalies)} features with significant drops in 202006:")
    anomaly_df = pl.DataFrame(anomalies).sort("drop_pct", descending=True)
    print("\nTop 20 features with biggest drops:")
    for i, row in enumerate(anomaly_df.head(20).iter_rows(named=True)):
        print(f"  {i+1}. {row['feature']}: {row['drop_pct']:.1f}% drop (other: {row['mean_other']:.2f}, June: {row['mean_june']:.2f})")
else:
    print("\n[OK] No significant feature drops detected in 202006")
...
# ============= CREATE HTML OUTPUT =============
print("\n[INFO] Creating interactive HTML heatmap...")
...
<head>
    <meta charset="utf-8">
    <title>Feature Mean Values Heatmap - PREPROCESSED DATA (After MICE)</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Feature Mean Values Heatmap - PREPROCESSED DATA</h1>
        <div class="subtitle">
            After MICE imputation + IPC correction<br>
            Normalized per feature (Red = Low values, Green = High values)<br>
            Blue column highlights foto_mes 202006
        </div>
        
        <div class="info-banner">
            <strong>Note:</strong> This data has been preprocessed with:
            <ul style="margin: 5px 0;">
                <li>MICE imputation (202006, 201905, 201910)</li>
                <li>IPC (inflation) correction for monetary columns</li>
                <li>Removed features: cprestamos_personales, mprestamos_personales</li>
            </ul>
        </div>
        ...
        <div class="controls">
            ...
            <div class="control-group">
                <button onclick="scrollFeatures(-50)">Up 50</button>
                <button onclick="scrollFeatures(50)">Down 50</button>
            </div>
        </div>
"""
...
if anomalies:
    html_output += """
        <div class="anomaly-list">
            <h3>[WARNING] Features with Significant Drops in 202006 (>80% drop)</h3>
            <p><em>These anomalies persist even after MICE imputation!</em></p>
"""
...
"""
    ...
    html_output += """
        <div style="margin-bottom: 20px; padding: 15px; background: #d4edda; border-left: 4px solid #28a745; border-radius: 5px; color: #155724;">
            <strong>[OK]</strong> No significant feature drops detected in 202006 after preprocessing!
        </div>
"""
...
# Save HTML file
output_path = Path('playground/feature_heatmap_preprocessed.html')
output_path.write_text(html_output, encoding='utf-8')

print(f"[OK] HTML heatmap saved to: {output_path.absolute()}")
print(f"   Open it in your browser to view the interactive heatmap!")
print(f"\n[INFO] Comparison:")
print(f"   - Original data:     playground/feature_heatmap.html")
print(f"   - Preprocessed data: playground/feature_heatmap_preprocessed.html")
...
def update_plot(start_idx):
    ...
    if june_idx >= 0:
        ax.axvline(x=june_idx, color='blue', linestyle='--', linewidth=3, label='202006 (MICE)')
    ...
    ax.set_title(f'PREPROCESSED DATA - Feature Mean Values Heatmap by foto_mes\n(After MICE + IPC | Red = Low, Green = High, Blue = 202006)\nShowing features {start_idx+1}-{end_idx} of {len(sample_features)}', 
...
if len(sample_features) > features_per_view:
    ...
    print("\n[INFO] Use the slider or UP/DOWN arrow keys to scroll through features")
